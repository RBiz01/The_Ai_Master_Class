...
3. Open `l04-interactive.html` and follow its Instructions until you hit the success state (this locks in: Tone, audience, and brand voice locks).
4. Capture a one-sentence Monday-morning takeaway you could paste into Slack.
5. **You’re done when:** you can explain — in plain language — *Lock tone, audience, and brand voice so outputs sound like Acme Ops—not a random chatbot—without overfitting to one person’s quirks.* and `l04-interactive.html` shows success.
