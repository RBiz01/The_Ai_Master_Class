...
3. Open `l02-interactive.html` and follow its Instructions until you hit the success state (this locks in: The Goal · Context · Constraints · Format frame).
4. Apply the idea once in your words (sticky note or note app is fine).
5. **You’re done when:** you can explain — in plain language — *Apply the **Goal · Context · Constraints · Format (GCCF)** frame to any work prompt so the model has a complete brief.* and `l02-interactive.html` shows success.
