...
3. Open `l02-interactive.html` and follow its Instructions until you hit the success state (this locks in: The retrieve → stuff → generate loop).
4. Apply the idea once in your words (sticky note or note app is fine).
5. **You’re done when:** you can explain — in plain language — *Map any knowledge-assistant turn onto retrieve → stuff → generate and name what can fail at each step.* and `l02-interactive.html` shows success.
