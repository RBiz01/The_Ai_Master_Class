...
3. Open `l02-interactive.html` and follow its Instructions until you hit the success state (this locks in: Chunk size, overlap, and boundaries).
4. Apply the idea once in your words (sticky note or note app is fine).
5. **You’re done when:** you can explain — in plain language — *Tune size and overlap so related sentences aren’t orphaned across chunk edges.* and `l02-interactive.html` shows success.
