...
3. Open `l04-interactive.html` and follow its Instructions until you hit the success state (this locks in: Debugging flaky actions).
4. Capture a one-sentence Monday-morning takeaway you could paste into Slack.
5. **You’re done when:** you can explain — in plain language — *Diagnose timeouts, auth stubs, and schema mismatches with a short checklist—without blaming the model first.* and `l04-interactive.html` shows success.
