...
3. Open `l02-interactive.html` and follow its Instructions until you hit the success state (this locks in: Tiny golden sets for assistants).
4. Apply the idea once in your words (sticky note or note app is fine).
5. **You’re done when:** you can explain — in plain language — *Build 8–15 prompts with expected traits (must include / must refuse) covering happy path and edges.* and `l02-interactive.html` shows success.
